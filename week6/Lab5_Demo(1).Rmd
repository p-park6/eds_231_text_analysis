...
The next step is to normalize these probabilities, that is, to calculate how often words occur together within a window, relative to their total occurrences in the data. We'll also harmonize the naming conventions from the different functions we used.
...
Then we cast to a matrix so we can use matrix factorization and reduce the dimensional of the data.
...
```{r syn_function}
search_synonyms <- function(word_vectors, selected_vectors, original_word){
  dat <- word_vectors %*% selected_vectors
...
```{r find-synonyms}
fall <- search_synonyms(word_vectors = word_vectors, 
                        selected_vectors = word_vectors["fall",], 
                        original_word = "fall")
fall
...
slip %>% 
  ...
  coord_flip() +
  labs(x = NULL,
       title = "Which word vectors are most similar to flip or fall")
```
