...
toks <- tokens_select(toks, pattern = add_stops, selection = "remove")
```
...
We are looking at plots to see where the 'sweet spot' is. In the graph, number 4 and number 8 are what pop out the most.
...
topic_names <- apply(topic_words, 2, paste, sep = " ")
```
