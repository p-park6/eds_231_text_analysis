...
incidents_split <- initial_split(incidents2class,strata = fatal) # equal proportion of fatal and nonfatal observations, both test and training have equal proportion of fatal to nonfatal 
...
```{r recipe}
incidents_recipe <- recipe(fatal ~ Text, data = incidents_train)

recipe <- incidents_recipe %>% 
  step_tokenize(Text) %>% 
  step_tokenfilter(Text,  max_tokens = 1000) %>% 
  # how might it make the frequency more normalized, stop words that makes them not useful, common across,correct frequency
  step_tfidf(Text) # traditional stop words, and domain specific stop words
```
...
```{r confusion-matrix}
conf_mat_resampled(nb_result, tidy = F) %>% 
