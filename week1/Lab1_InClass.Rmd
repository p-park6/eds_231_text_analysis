...
API_KEY <- "7aMhkl8aJMmmZTC70FDDG8jCULWJgnLa"
```
...
for(i in 1:maxPages){
  ...
  pages[[i+1]] <- nytSearch #up to here, we are requesting for all of these pages all at once. Because we are 'maxing out' the amount of requests we are sending, it doesn't let us get these pages
  Sys.sleep(12) #this slows down how often we are asking for these pages. we are slowing it down, giving it time to 'recover' and letting us get all the pages we would like without it erroring out
}
```
